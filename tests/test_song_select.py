...
class TestSongSelect(ModeTestCase):
    ...
    def test_temp_show_not_running(self):
        "Temporary test to make sure that "
        self.setup_game()

        self.advance_time_and_run(1)

        # this is what the player var is called, it is NOT equal to the event name that we listen for
        self.assertPlayerVarEqual(1, 'shot_sh_song_select_bash_center_enabled')
        # state of the shot is stored here
        self.assertPlayerVarEqual(0, 'shot_sh_song_select_bash_center')

        self.machine.shots.sh_song_select_bash_center.jump(1)
        self.advance_time_and_run(1)
        self.assertPlayerVarEqual(1, 'shot_sh_song_select_bash_center')
        self.post_event('state_song_select_wait_started')

        self.advance_time_and_run(1)
        self.assertModeNotRunning('song_select')
        self.assertPlayerVarEqual(0, 'shot_sh_song_select_bash_center_enabled')
        self.assertModeRunning('autostack')
